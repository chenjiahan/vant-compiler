...
const templateCompilerES2015 = require('vue-template-es2015-compiler');
const templateCompilerConfig = { preserveWhitespace: false };

module.exports = function(code) {
  const { script, template } = templateCompiler.parseComponent(code);
  const renderFunction = parseTemplate(template.content);
  return renderJs(script.content, renderFunction);
}

// 将 template html 解析为 render 函数
function parseTemplate(template) {
  const compiled = templateCompiler.compile(template, templateCompilerConfig);
...
// 插入 render 函数
function renderJs(rawScript, render) {
